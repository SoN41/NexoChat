...
const Message = ({ message }) => {
  ...
  // Better bubble colors 
  const bubbleBgColor = fromMe ? 'bg-blue-600' : 'bg-gray-700';
  const shakeClass = message.shouldShake ? "shake" : "";
  
  // Check what content the message contains
  ...
  return (
    <div className={`chat ${chatClassName} mb-4`}>
        <div className="chat-image avatar">
            <div className="w-10 rounded-full border border-gray-600 shadow-sm">
                <img src={profilePic} alt="profile" />
            </div>
        </div>
        
        {/* Dynamic padding: tight if pure image, standard if it has text */}
        <div className={`chat-bubble text-white ${bubbleBgColor} ${shakeClass} shadow-md flex flex-col ${hasImage && !hasText ? 'p-1.5' : 'py-2 px-3'}`}>
